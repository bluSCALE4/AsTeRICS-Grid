...
function init() {
    ...
    if (
        localStorageService.getUserMajorModelVersion(autologinUser) >
        modelUtil.getLatestModelVersion().major
    ) {
        log.info(
    ...
    if (urlParamService.isDemoMode()) {
        promises.push(
            loginService.registerOffline(
                constants.LOCAL_DEMO_USERNAME,
                constants.LOCAL_DEMO_USERNAME
            )
        );
        localStorageService.setAutologinUser("");
    ...
    Promise.all(promises)
        ...
        .then(() => {
            ...
            let toLogin =
                lastActiveUser ||
                localStorageService.getSavedUsers().length > 0;
            localStorageService.setLastActiveUser(
                autologinUser || lastActiveUser || ""
            );
            let initHash =
                location.hash ||
                (toMain ? "#main" : toLogin ? "#login" : "#welcome");
        ...
        navigator.serviceWorker.register("./serviceWorker.js").then((reg) => {
            ...
            navigator.serviceWorker.addEventListener("message", (evt) => {
                if (isUpdate && evt.data && evt.data.activated) {
                    MainVue.setTooltipI18n(
                        i18nService.t(
                            "newVersionAvailableTheNextTimeYoullUseUpdated"
                        ),
                        {
...
function checkAppVersion() {
    ...
    if (version && version !== constants.CURRENT_VERSION) {
        let showMsg = () => {
            let text = i18nService.t(
                "youreNowUsingVersion",
                constants.CURRENT_VERSION
            );
