...
i18nService.getVueI18n = async function () {
    ...
    getMetadataConfig();
    return i18nService
        .setAppLanguage(i18nService.getAppLang(), true)
        .then(() => {
...
i18nService.setAppLanguage = function (lang, dontSave) {
    ...
    return loadLanguage(useLang).then(() => {
        ...
        allLanguages.sort((a, b) =>
            a[useLang].toLowerCase() > b[useLang].toLowerCase() ? 1 : -1
        );
...
i18nService.getTranslation = function (i18nObject, options) {
    ...
    if (i18nObject[lang]) {
        return !options.includeLang
            ? i18nObject[lang]
            : { lang: lang, text: i18nObject[lang] };
    }
