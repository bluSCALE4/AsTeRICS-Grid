...
urlParamService.isScanningEnabled = function () {
    return (
        hasParam(urlParamService.params.PARAM_SCANNING) &&
        !isParamFalse(urlParamService.params.PARAM_SCANNING)
    );
};

urlParamService.isDirectionEnabled = function () {
    return (
        hasParam(urlParamService.params.PARAM_DIR_INPUT) &&
        !isParamFalse(urlParamService.params.PARAM_DIR_INPUT)
    );
};

urlParamService.isHuffmanEnabled = function () {
    return (
        hasParam(urlParamService.params.PARAM_HUFFMAN) &&
        !isParamFalse(urlParamService.params.PARAM_HUFFMAN)
    );
};
...
urlParamService.shouldResetDatabase = function () {
    let shouldReset =
        !_alreadyResetted &&
        (urlParamService.isDemoMode() ||
            isParamTrue(urlParamService.params.PARAM_RESET_DATABASE));
