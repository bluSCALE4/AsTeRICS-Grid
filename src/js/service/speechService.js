...
speechService.speak = function (textOrOject, options) {
    ...
    options.voiceLangIsTextLang =
        options.voiceLangIsTextLang || _voiceLangIsTextLang;
    let text = null;
    ...
    let alternativeLang =
        options.voiceLangIsTextLang && prefVoiceLang
            ? prefVoiceLang
    ...
    if (
        text === lastSpeakText &&
        new Date().getTime() - lastSpeakTime < options.minEqualPause
    ) {
        return;
    }
    lastSpeakText = text;
    ...
    let nativeVoices = voices.filter(
        (voice) => voice.type === speechService.VOICE_TYPE_NATIVE
    );
    let responsiveVoices = voices.filter(
        (voice) => voice.type === speechService.VOICE_TYPE_RESPONSIVEVOICE
    );
    if (speechService.nativeSpeechSupported() && nativeVoices.length > 0) {
        ...
        msg.pitch =
            isSelectedVoice && !options.useStandardRatePitch ? _voicePitch : 1;
        msg.rate =
            options.rate ||
            (isSelectedVoice && !options.useStandardRatePitch ? _voiceRate : 1);
...
speechService.isSpeaking = function () {
    return (
        (speechService.nativeSpeechSupported() &&
...
speechService.testSpeak = function (voiceName, testSentence, testLang) {
    ...
    let voiceLang = speechService
        .getVoices()
        .filter((lang) => lang.name === voiceName)[0].lang;
...
speechService.getVoicesLangs = function () {
    ...
    return i18nService
        .getAllLanguages()
        .filter((lang) => voiceLangCodes.indexOf(lang.code) !== -1);
};
...
speechService.voiceSortFn = function (a, b) {
    if (a.lang !== b.lang) {
        return i18nService
            .t(`lang.${a.lang}`)
            .localeCompare(i18nService.t(`lang.${b.lang}`));
    }
    if (a.type !== b.type) {
    ...
    let aSortBack = voiceSortBackList.some((id) =>
        a.id.toLowerCase().includes(id.toLowerCase())
    );
    let bSortBack = voiceSortBackList.some((id) =>
...
function addVoice(voiceName, voiceLang, voiceType, voiceReference) {
    ...
    let existingNameIndex = allVoices
...
async function registerVoices(arrayNativeVoices) {
    arrayNativeVoices.forEach((voice) => {
        addVoice(
            voice.name,
            voice.lang,
            speechService.VOICE_TYPE_NATIVE,
            voice
        );
    });
}
...
async function init() {
    ...
    responsiveVoiceVoices.forEach((voice) => {
        addVoice(
            voice.name,
            voice.lang,
            speechService.VOICE_TYPE_RESPONSIVEVOICE
        );
    });
}
init();
