...
let initYtState = {
    ...
    lastData:
        "https://www.youtube.com/watch?v=5ffLB4a9APc&list=PL0UXHkT03dGrIHldlEKR0ZWfNMkShuTNz",
    lastVideoId: null,
...
let playerID = "player";
let ytState =
    localStorageService.getYTState() || JSON.parse(JSON.stringify(initYtState));
...
youtubeService.play = function (action, videoTimeParam) {
    ...
    promise.then(() => {
            ...
            player = new YT.Player(playerID, {
                ...
                events: {
                    ...
                    onStateChange: (event) => {
                        if (
                            (waitForBuffering &&
                                event.data === PLAYER_STATES.PLAYING) ||
            ...
            switch (action.playType) {
                case GridActionYoutube.playTypes.YT_PLAY_VIDEO:
                    ...
                    player.loadVideoById(
                        videoId,
                        getVideoTime(videoTimeParam, videoId)
                    );
                    break;
                case GridActionYoutube.playTypes.YT_PLAY_SEARCH:
                    ...
                    callGapiCached("gapi.client.youtube.search.list", {
                        ...
                        player.loadPlaylist(
                            videoIds,
                            ytState.lastPlaylistIndexes[action.data]
                        );
                        setTimeout(() => {
                            if (!player) return;
                ...
                case GridActionYoutube.playTypes.YT_PLAY_CHANNEL:
                    ...
                    let channelPlaylist =
                        youtubeService.getChannelPlaylist(channel);
                    waitForBuffering = true;
                    player.loadPlaylist({
        ...
        function onBuffering() {
            ...
            let seekTime = getVideoTime(
                videoTimeParam,
                youtubeService.getCurrentVideoId()
            );
            if (seekTime) {
...
youtubeService.volumeUp = function (diffPercentage) {
    youtubeService.setVolume(
        Math.min(player.getVolume() + diffPercentage, 100)
    );
};
...
youtubeService.getVideoId = function (videoLink) {
    ...
    if (vParam) {
        ...
        let startIndex =
            videoLink.indexOf(shortUrlPrefix) + shortUrlPrefix.length;
        let endIndex =
            videoLink.indexOf("?") !== -1 ? videoLink.indexOf("?") : undefined;
...
youtubeService.getChannelId = function (link) {
    ...
    for (let i = 0; i < channelPrefixes.length; i++) {
        if (link.indexOf(channelPrefixes[i]) !== -1) {
            let start =
                link.indexOf(channelPrefixes[i]) + channelPrefixes[i].length;
...
function saveState() {
    if (player && player.getPlaylistIndex) {
        ...
        if (
            currentIndex >= 0 &&
            ytState.lastPlayType !== GridActionYoutube.playTypes.YT_PLAY_VIDEO
        ) {
...
function init() {
    ...
    $(document).on(constants.EVENT_USER_CHANGED, () => {
        ytState =
            localStorageService.getYTState() ||
            JSON.parse(JSON.stringify(initYtState));
    });
