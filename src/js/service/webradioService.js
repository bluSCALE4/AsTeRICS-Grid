...
let volume = parseFloat(
    localStorageService.get(WEBRADIO_LAST_VOLUME_KEY) || 1.0
);
...
webradioService.doAction = function (gridId, action) {
    dataService.getGrid(gridId).then((grid) => {
        ...
        let webradio =
            radios.filter((radio) => radioId === radio.radioId)[0] || radios[0];
        let index = 0;
        if (!webradio) {
...
webradioService.play = function (webradio) {
    ...
    fillUrl(webradio).then((radioWithUrl) => {
        ...
        let tooltipText = i18nService.t(
            "playingWebradio",
            radioWithUrl.radioName
        );
...
webradioService.toggle = function (webradio) {
    if (
        (!playingVideo && player.paused) ||
        (playingVideo && videoPlayer.paused)
    ) {
...
webradioService.search = function (searchString, limit, offset) {
    ...
    paramPositions.forEach((paramPosition, index, array) => {
        ...
        let finalParam = searchString.substring(
            paramPosition.position + paramPosition.param.length + 1,
            nextPos
        );
    ...
    return new Promise((resolve, reject) => {
        $.ajax({
            ...
            (data) => {
                ...
                let workingRadios = data.filter(
                    (radio) => radio.lastcheckok !== "0"
                );
...
function fillUrl(webradio, gridId) {
    return new Promise((resolve, reject) => {
        ...
        $.ajax({
            ...
            if (
                webradio.radioUrl.lastIndexOf("/") ===
                webradio.radioUrl.length - 1
            ) {
            ...
            if (gridId) {
                dataService.getGrid(gridId).then((grid) => {
                    let radios = grid.webRadios || [];
                    let savedRadio = radios.filter(
                        (radio) => webradio.radioId === radio.radioId
                    )[0];
                    if (savedRadio) {
