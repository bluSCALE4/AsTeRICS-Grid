...
test("encryptionService.encryptObject - Test 3", () => {
    ...
    let result = encryptionService.encryptObject(object);
    expect(result instanceof EncryptedObject).toBeTruthy();
    expect(result.encryptedDataBase64).toEqual(
        DEFAULT_SALT + encryptionKey + json
    );
    expect(result.encryptedDataBase64Short).toEqual(null);
});
...
test("encryptionService.decryptObject - shortening", () => {
    ...
    let result = encryptionService.encryptObject(object);
    expect(
        encryptionService.decryptObjects(result, { onlyShortVersion: false })
            .data
    ).toEqual(object.data);
    expect(
        encryptionService.decryptObjects(result, { onlyShortVersion: true })
            .data
    ).toEqual(objectShortened.data);
});
