 ...
 */
pouchDbService.initDatabase = function (
    databaseName,
    remoteCouchDbAddress,
    onlyRemote
) {
    if (
        _pouchDbAdapter &&
        _pouchDbAdapter.getOpenedDatabaseName() === databaseName &&
        remoteCouchDbAddress
    ) {
    ...
    _pouchDbAdapter = new PouchDbAdapter(
        databaseName,
        remoteCouchDbAddress,
        onlyRemote,
        false,
 ...
 * @param onlyRemote
 */
pouchDbService.createDatabase = function (
    databaseName,
    remoteCouchDbAddress,
    onlyRemote
) {
...
pouchDbService.all = function (idPrefix, id) {
    if (id && _documentCache.has(id)) {
        ...
        log.debug(
            "using cache for retrieving bulk with id-prefix: " + idPrefix
        );
...
pouchDbService.save = function (idPrefix, data) {
    ...
    return new Promise((resolve, reject) => {
        if (!data || !data._id || !idPrefix || !data.encryptedDataBase64) {
            log.error(
                'did not specify needed parameter "idPrefix" or "_id" or data is not encrypted! aborting.'
            );
            return reject();
        }
        _pouchDbAdapter
            ...
            .catch(function (err) {
                ...
                if (err.error === "conflict") {
                    log.warn(
                        "conflict with remote version updating document with id: " +
                            data.id
                    );
                    resolve();
                } else {
...
pouchDbService.resetDatabase = function (databaseName) {
    if (
        !pouchDbService.isUsingLocalDb() ||
        pouchDbService.getOpenedDatabaseName() !== constants.LOCAL_DEMO_USERNAME
    ) {
...
pouchDbService.deleteDatabase = function (databaseName) {
    if (
        (pouchDbService.getOpenedDatabaseName() === databaseName &&
            !pouchDbService.isUsingLocalDb()) ||
        !databaseName
    ) {
        log.warn(
            "won't delete database since using remote db or databaseName not specified..."
        );
        return Promise.reject();
    }
    _documentCache.clearAll();
...
function resumeSyncInternal() {
    let timeout = getPouchDbAdapter().wasCurrentDatabaseSynced()
        ? 0
        : _lastQueryTime + 1000;
...
function changeHandler(changedIds, changedDocsEncrypted) {
    ...
    let changedDocs = changedDocsEncrypted.map((rawDoc) =>
        filterService.convertDatabaseToLiveObjects(rawDoc)
    );
    let user = pouchDbService.getOpenedDatabaseName();
    let currentUserDataModelVersion =
        localStorageService.getUserMajorModelVersion(user);
    changedDocs.forEach((doc) => {
        localStorageService.setUserModelVersion(user, doc.modelVersion);
    });
    if (
        currentUserDataModelVersion ===
        localStorageService.getUserMajorModelVersion(user)
    ) {
