...
function PouchDbAdapter(
    ...
    thiz.init = function () {
        ...
        if (!remoteCouchDbAddress || (remoteCouchDbAddress && !onlyRemote)) {
            _openLocalPromise = openDbInternal(databaseName).then(
                (dbHandler) => {
                    _db = dbHandler;
    ...
    thiz.startSync = function (remoteCouchDbAddress) {
        if (_remoteDb || !_openLocalPromise) {
            log.warn(
                "startSync() is not possible if remote database is already open, or local database not opened"
            );
            return;
        }
    ...
    function setupSync() {
        if (!_remoteDb || !_db) {
            log.error(
                "trying to setupSync() but remoteDb or db is not specified! Aborting..."
            );
            return Promise.reject();
        }
        return new Promise((resolve) => {
            ...
            if (!thiz.wasCurrentDatabaseSynced() && !justCreated) {
                ...
                log.debug(
                    "database wasn't synced before, so temporarily use remote db until sync is done..."
                );
                _useLocalDb = false;
                _syncHandler = _db
                    .sync(_remoteDb, {
                    ...
                    .on("complete", function (info) {
                        ...
                        if (status === "cancelled") {
                            ...
                            log.info(
                                "couchdb sync complete! setting up live sync and using local db now..."
                            );
                            log.debug(
                                "initial sync took: " +
                            ...
                            setupLiveSync();
                            $(document).trigger(
                                constants.EVENT_DB_INITIAL_SYNC_COMPLETE
                            );
                        }
                    });
                resolve();
        ...
        try {
            ...
            if (info.direction && info.direction === "pull") {
                if (
                    info.change &&
                    info.change.docs &&
                    info.change.docs.length > 0
                ) {
                    changedDocsEncrypted = info.change.docs.filter(
                        (doc) => !!getId(doc) && !isOutdatedRevision(doc)
                    );
            ...
            if (
                !_closed &&
                (changedIds.length > 0 || changedDocsEncrypted.length > 0)
            ) {
    ...
    function isOutdatedRevision(doc) {
        return (
            _revisionMap[getId(doc)] &&
            getRevNumber(doc) < _revisionMap[getId(doc)]
        );
    }
