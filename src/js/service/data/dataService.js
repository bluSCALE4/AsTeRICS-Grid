...
if (urlParamService.getDefaultGridsetName()) {
    _defaultGridSetPath =
        "app/examples/" + urlParamService.getDefaultGridsetName();
}
...
dataService.getGrids = function (fullVersion, withoutGlobal) {
    return new Promise((resolve) => {
        databaseService
            .getObject(GridData, null, !fullVersion)
            .then((grids) => {
                if (!grids) {
                    resolve([]);
                    return;
                ...
                if (withoutGlobal) {
                    dataService.getMetadata().then((metadata) => {
                        resolve(
                            retVal.filter(
                                (grid) =>
                                    grid && grid.id !== metadata.globalGridId
                            )
                        );
                    });
                } else {
...
dataService.saveGrids = function (gridDataList) {
    gridDataList.forEach((gridData) => {
        gridData.gridElements = gridUtil.sortGridElements(
        ...
        dataService.getMetadata().then((existingMetadata) => {
            ...
            if (!existingMetadata.isEqual(newMetadata)) {
                ...
                databaseService.saveObject(MetaData, newMetadata).then(() => {
                    resolve();
                    $(document).trigger(
                        constants.EVENT_METADATA_UPDATED,
                        newMetadata
                    );
                });
            } else {
        ...
        databaseService.getObject(MetaData).then((result) => {
            ...
            if (!localStorageService.shouldSyncNavigation()) {
                ...
                if (localMetadata) {
                    ...
                    returnValue.lastOpenedGridId =
                        localMetadata.lastOpenedGridId;
                }
            }
...
dataService.downloadToFile = async function (gridIds, options) {
    ...
    exportData.grids = allGrids.filter(
...
dataService.normalizeImportData = function (data) {
    ...
    importData.dictionaries = filterService.updateDataModel(
        importData.dictionaries
    );
    if (importData.metadata) {
        importData.metadata = filterService.updateDataModel(
            importData.metadata
        );
    }
    return importData;
...
dataService.importData = async function (data, options) {
    ...
    if (options.generateGlobalGrid && !importData.metadata.globalGridId) {
        ...
        let globalGrid = gridUtil.generateGlobalGrid(
            homeGridId,
    ...
    if (options.importUserSettings) {
        ...
        existingMetadata.lastOpenedGridId =
            importData.metadata.lastOpenedGridId;
    ...
    if (importData.metadata) {
        importData.metadata.globalGridActive =
            !!importData.metadata.globalGridId;
