                ...
                .then((result) => {
                    ...
                    if (
                        ...
                        localStorageService.setUserModelVersion(
                            pouchDbService.getOpenedDatabaseName(),
                            modelVersion
                        );
                    }
...
databaseService.getSingleObject = function (objectType, id, onlyShortVersion) {
    return databaseService
        .getObject(objectType, id, onlyShortVersion)
        .then((result) => {
    ...
    return _initPromise
        .then(() => {
            ...
            if (data.isShortVersion) {
                log.warn(
                    "short versions of objects cannot be saved/updated! aborting."
                );
                return Promise.reject();
            }
        ...
        .then((existingObject) => {
            if (existingObject) {
                log.debug(
                    objectType.getModelName() +
                        " already existing, doing update. id: " +
                        existingObject.id
                );
 ...
 * @return {*}
 */
databaseService.initForUser = function (
    username,
    hashedUserPassword,
    userDatabaseURL,
    onlyRemote
    ...
    let isLocalUser =
        localStorageService.getSavedLocalUsers().indexOf(username) !== -1;
    ...
    return pouchDbService
        .initDatabase(username, userDatabaseURL, onlyRemote)
        .then(() => {
 ...
 * @return {*}
 */
databaseService.registerForUser = function (
    username,
    hashedUserPassword,
    userDatabaseURL,
    onlyRemote
    ...
    let isLocalUser =
        localStorageService.getSavedLocalUsers().indexOf(username) !== -1;
    if (
        pouchDbService.getOpenedDatabaseName() === username &&
        shouldSync === pouchDbService.isSyncEnabled()
    ) {
        return Promise.resolve();
    }
    return pouchDbService
        .createDatabase(username, userDatabaseURL, onlyRemote)
        .then(() => {
            return initInternal(hashedUserPassword, username, isLocalUser);
        });
};
    ...
    _initPromise = Promise.resolve()
        ...
        .then((metadataObjects) => {
            ...
            if (metadataObjects.length === 0) {
                ...
                encryptionService.setEncryptionProperties(
                    hashedUserPassword,
                    metadata.id,
                    isLocalUser
                );
                promises.push(
                    applyFiltersAndSave(MetaData.getIdPrefix(), metadata)
                );
            }
