...
encryptionService.encryptObject = function (object) {
    ...
    let shortJsonString = JSON.stringify(
        dataUtil.removeLongPropertyValues(object)
    );
    let shortVersionDifferent = jsonString !== shortJsonString;
    encryptedObject.encryptedDataBase64 = encryptionService.encryptString(
        jsonString,
        _encryptionSalts[0]
    );
...
encryptionService.decryptObjects = function (encryptedObjects, options) {
    ...
    encryptedObjects =
        encryptedObjects instanceof Array
            ? encryptedObjects
            : [encryptedObjects];
    ...
    encryptedObjects.forEach((encryptedObject) => {
        try {
            ...
            if (onlyShortVersion) {
                let toDecrypt =
                    encryptedObject.encryptedDataBase64Short ||
...
encryptionService.decryptStringTrySalts = function (encryptedString, trySalts) {
    try {
        ...
        if (trySalts.length === 0) {
            log.error(
                "wasn't able to decrypt string, no remaining salts for trying!"
            );
            throw e;
        }
        log.warn("wasn't able to decrypt string, try next salt...");
        return encryptionService.decryptStringTrySalts(
            encryptedString,
            trySalts
        );
    }
...
encryptionService.getUserPasswordHash = function (plaintextPassword) {
    return encryptionService.getStringHash(
        STATIC_USER_PW_SALT + plaintextPassword
    );
};
 ...
 */
encryptionService.setEncryptionProperties = function (
    hashedPassword,
    salts,
    isLocalUser
) {
...
function throwErrorIfUninitialized() {
    if (
        !_encryptionBasePassword ||
        !_encryptionSalts ||
        _encryptionSalts.length === 0
    ) {
        let msg =
            "using encryptionService uninitialized is not possible, aborting...";
        log.error(msg);
        throw msg;
    }
}
