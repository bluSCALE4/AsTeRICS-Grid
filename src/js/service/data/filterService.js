...
filterService.convertLiveToDatabaseObjects = function (objects, filterOptions) {
    ...
    let filtered = filterObjects(
        objects,
        filterOptions,
        getFilterFunctionsToDatabase
    );
...
filterService.convertDatabaseToLiveObjects = function (objects, filterOptions) {
    ...
    let filtered = filterObjects(
        objects,
        filterOptions,
        getFilterFunctionsFromDatabase
    );
...
function filterObjects(objects, filterOptions, getFilterFunctionsFunction) {
    ...
    for (let i = 0; i < objects.length; i++) {
        let filterFunctions = getFilterFunctionsFunction(
            modelUtil.getModelVersionObject(objects[i].modelVersion)
        );
    ...
    switch (objectModelVersion.major) {
        case 1:
            filterFns.push(function (object, filterOptions) {
                ...
                if (object.modelName === MetaData.getModelName()) {
                    log.info(
                        "converting model version from V1 to V2: " +
                            object.modelName
                    );
        ...
        case 2:
            filterFns.push(function (gridData, filterOptions) {
                ...
                if (gridData.modelName === GridData.getModelName()) {
                    log.debug(
                        "converting model version from V2 to V3: " +
                            gridData.modelName
                    );
                    let locale =
                        gridData.locale || i18nService.getContentLang();
        ...
        case 3:
            filterFns.push(function (gridData, filterOptions) {
                ...
                if (gridData.modelName === GridData.getModelName()) {
                    log.debug(
                        "converting model version from V3 to V4: " +
                            (gridData.label ? gridData.label.de : "")
                    );
                    for (let i = 0; i < gridData.gridElements.length; i++) {
                        let gridElement = gridData.gridElements[i];
                        if (
                            gridElement.type ===
                            GridElement.ELEMENT_TYPE_COLLECT
                        ) {
                            let collectElem = new GridElementCollect();
                            gridElement.imageHeightPercentage =
                                gridElement.imageHeightPercentage
                                    ? parseInt(
                                          gridElement.imageHeightPercentage
                                      )
