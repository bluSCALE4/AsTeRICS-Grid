...
areService.getComponentDescriptorsAsXml = function (areURI) {
    return new Promise((resolve, reject) => {
        $.ajax({
            ...
            url:
                areService.getRestURL(areURI) +
                "storage/components/descriptors/xml",
            datatype: "text/xml",
            crossDomain: true,
            success: function (data, textStatus, jqXHR) {
...
areService.subscribeEvents = function (areURI, eventCallback, errorCallback) {
    ...
    if (!_eventSourceMap[areUrl]) {
        _eventSourceMap[areUrl] = new EventSource(
            areUrl + "runtime/model/channels/event/listener"
        ); // Connecting to SSE service for event channel events
