...
collectElementService.doCollectElementActions = async function (action) {
    ...
    switch (action) {
        case GridActionCollectElement.COLLECT_ACTION_SPEAK:
            if (
                autoCollectImage ||
                collectMode === GridElementCollect.MODE_COLLECT_SEPARATED
            ) {
        ...
        case GridActionCollectElement.COLLECT_ACTION_SPEAK_CLEAR:
            if (
                ...
                speechService.speakArray(
                    getSpeakTextObjectArray(),
                    (index, finished) => {
                        markedImageIndex = index;
                        updateCollectElements();
                        if (finished) {
        ...
        case GridActionCollectElement.COLLECT_ACTION_REMOVE_WORD:
            ...
            if (removedLabel) {
                collectedText = collectedText.substring(
                    ...
                    collectedText
                        .toLowerCase()
                        .lastIndexOf(removedLabel.toLowerCase())
                );
            }
    ...
    for (let collectElement of registeredCollectElements) {
        ...
        if (!imageMode) {
            $(`#${collectElement.id}`).attr(
                "aria-label",
                `${collectedText}, ${i18nService.t("ELEMENT_TYPE_COLLECT")}`
            );
            ...
            $(`#${collectElement.id}`).attr(
                ...
                `${collectedElements
                    .map((e) => getLabel(e))
                    .join(" ")}, ${i18nService.t("ELEMENT_TYPE_COLLECT")}`
            );
            let html = "";
            let height =
                $(`#${collectElement.id} .collect-container`).prop(
                    "clientHeight"
            ...
            let imgContainerHeight = showLabel
                ? height * imagePercentage
                : height;
            let imageRatios = [];
            ...
            let maxImages = Math.floor(
                width / (imgContainerHeight * maxImgRatio)
            );
            let numLines = 1;
            while (maxImages < imageCount && !useSingleLine) {
                ...
                maxImages =
                    Math.floor(
                        width / ((imgContainerHeight * maxImgRatio) / numLines)
                    ) * numLines;
            }
            ...
            for (const [
                ...
                if (image) {
                    ...
                    let fontSizeFactor =
                        collectElement.textElemSizeFactor || 1.5;
            ...
            if (useSingleLine) {
                ...
                $(`#${collectElement.id} .collect-container`).scrollLeft(
                    scroll
                );
...
$(window).on(constants.ELEMENT_EVENT_ID, function (event, element) {
    ...
    if (label && element.type === GridElement.ELEMENT_TYPE_NORMAL) {
        let textToAdd =
            label.length === 1 && keyboardLikeFactor > 0.4
                ? label
                : label + " ";
        ...
        if (word) {
            let appliedText = predictionService.applyPrediction(
                collectedText || "",
                word,
                dictionaryKey
            );
...
function triggerPredict() {
    registeredCollectElements.forEach((collectElement) => {
        let predictAction = getActionOfType(
            collectElement,
            "GridActionPredict"
        );
...
async function getMetadataConfig() {
    ...
    duplicatedCollectPause =
        metadata.inputConfig.globalMinPauseCollectSpeak || 0;
