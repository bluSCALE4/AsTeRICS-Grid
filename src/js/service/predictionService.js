...
predictionService.predict = function (input, dictionaryKey) {
    if (
        input === undefined ||
        registeredPredictElements.length === 0 ||
        !predictionary
    ) {
        return;
    }
    ...
    for (let i = 0; i < registeredPredictElements.length; i++) {
        ...
        $(`#${registeredPredictElements[i].id} .text-container span`).text(
            text
        );
...
predictionService.learnFromInput = function (input, dictionaryKey) {
    ...
    _unsavedChanges =
        predictionary.learnFromInput(input, dictionaryKey) || _unsavedChanges;
};
...
predictionService.applyPrediction = function (
    input,
    prediction,
    dictionaryKey
) {
...
predictionService.doAction = function (elementId) {
    ...
    let element = registeredPredictElements.filter(
...
function saveDictionaries() {
    ...
    _usedKeys.forEach((key) => {
        let dbDict =
            _dbDictObjects.filter((el) => el.dictionaryKey === key)[0] ||
            new Dictionary({ dictionaryKey: key });
...
$(document).on(
    constants.EVENT_DB_PULL_UPDATED,
    (event, updatedIds, updatedDocs) => {
