...
loginService.loginStoredUser = function (user, dontRoute) {
    ...
    let promise = new Promise((resolve, reject) => {
        ...
        if (
            savedOnlineUsers.includes(user) &&
            localStorageService.isDatabaseSynced(user)
        ) {
            ...
            log.info(
                "waiting for successful login because user wasn't completely synced before..."
            );
            let password = localStorageService.getUserPassword(user);
            loginService
...
loginService.validateUsername = function (username) {
    return new Promise((resolve, reject) => {
        ...
        if (
            localStorageService.isSavedLocalUser(username) ||
            username === constants.LOCAL_DEMO_USERNAME
        ) {
        ...
        (reason) => {
            ...
            if (localStorageService.isDatabaseSynced(user)) {
                ...
                if (
                    ...
                    loginService.ERROR_CODE_UNAUTHORIZED
                ) {
                    autoRetryLogin(user, hashedPassword, saveUser);
                }
                return databaseService
                    .initForUser(user, hashedPassword)
                    .then(() => {
...
function reasonToErrorCode(reason) {
    ...
    if (
        reason &&
        reason.message &&
        reason.message.toLowerCase() === "network error"
    ) {
