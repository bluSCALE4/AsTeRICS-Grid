...
function addGridToPdf(doc, gridData, options, metadata) {
    ...
    let hasARASAACImages = gridData.gridElements.reduce(
        (total, element) =>
            total ||
            (element.image &&
                element.image.searchProviderName ===
                    arasaacService.SEARCH_PROVIDER_NAME),
        false
    );
    let registerHeight = options.showRegister && options.pages > 1 ? 10 : 0;
    let footerHeight = hasARASAACImages
        ? 2 * pdfOptions.footerHeight
        : pdfOptions.footerHeight;
    let elementTotalWidth =
        (DOC_WIDTH - 2 * pdfOptions.docPadding) / gridData.getWidth();
    let elementTotalHeight =
        (DOC_HEIGHT -
            2 * pdfOptions.docPadding -
            footerHeight -
            registerHeight) /
        gridData.getHeight();
    if (footerHeight > 0) {
        ...
        let yLine1 = hasARASAACImages
            ? yBaseFooter - pdfOptions.footerHeight
            : yBaseFooter;
        if (options.showLinks && firstParentPage) {
            let prefix = JSON.stringify(
                options.idParentsMap[gridData.id].slice(0, 5)
            );
            textC = prefix + " => " + textC;
        ...
        doc.text(
            textL,
            pdfOptions.docPadding + pdfOptions.elementMargin,
            yLine1,
            {
                baseline: "bottom",
                align: "left"
        ...
        doc.text(
            textR,
            DOC_WIDTH - pdfOptions.docPadding - pdfOptions.elementMargin,
            yLine1,
            {
                baseline: "bottom",
                align: "right"
    ...
    gridData.gridElements.forEach((element) => {
        ...
        let currentWidth =
            elementTotalWidth * element.width - 2 * pdfOptions.elementMargin;
        let currentHeight =
            elementTotalHeight * element.height - 2 * pdfOptions.elementMargin;
        let xStartPos =
            pdfOptions.docPadding +
            elementTotalWidth * element.x +
            pdfOptions.elementMargin;
        let yStartPos =
            pdfOptions.docPadding +
            elementTotalHeight * element.y +
            pdfOptions.elementMargin;
        doc.setDrawColor(0);
        if (!options.printBackground) {
            doc.setFillColor(255, 255, 255);
        } else {
            let colorRGB = util.getRGB(
                MetaData.getElementColor(element, metadata)
            );
        ...
        promises.push(
            addImageToPdf(
                ...
                if (
                    options.showLinks &&
                    options.idPageMap[element.getNavigateGridId()]
                ) {
                    let targetPage =
                        options.idPageMap[element.getNavigateGridId()];
...
function addLabelToPdf(
    ...
    let fontSizeMM = hasImg
        ? currentHeight * (1 - pdfOptions.imgHeightPercentage)
        : currentHeight / 2;
    ...
    let yOffset = hasImg
        ? currentHeight - 2 * pdfOptions.elementMargin
        : (currentHeight - dim.h * lines) / 2;
...
function getOptimalFontsize(
    ...
    for (let i = 0; i < steps; i++) {
        ...
        if (multipleLines && text.indexOf(" ") !== -1) {
            ...
            if (
                dim.w / possibleLines > maxWidth * 0.5 ||
                currentLines > possibleLines
            ) {
                size -= stepSize;
            } else {
                size += stepSize;
...
async function addImageToPdf(
    ...
    let hasImage =
        element && element.image && (element.image.data || element.image.url);
    if (!hasImage) {
        return Promise.resolve();
    }
    ...
    let dim = null;
    if (!imageData) {
        let dataWithDim = await imageUtil.urlToBase64WithDimensions(
            element.image.url,
            500,
            type
        );
    ...
    let imgHeightPercentage = i18nService.getTranslation(element.label)
        ? pdfOptions.imgHeightPercentage
        : 1;
    let maxWidth = elementWidth - 2 * pdfOptions.imgMargin;
    let maxHeight =
        (elementHeight - 2 * pdfOptions.imgMargin) * imgHeightPercentage;
    ...
    if (type === GridImage.IMAGE_TYPES.PNG) {
        ...
        let pngBase64 = await imageUtil.base64SvgToBase64Png(
            imageData,
            pixelWidth
        );
