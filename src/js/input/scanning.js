...
Scanner.getInstanceFromConfig = function (
    inputConfig,
    itemSelector,
    scanActiveClass,
    scanInactiveClass
    ...
    return new ScannerConstructor(itemSelector, scanActiveClass, {
        ...
        scanTimeoutFirstElementFactor:
            inputConfig.scanTimeoutFirstElementFactor,
        touchScanning: !inputConfig.mouseclickEnabled,
        inputEventSelect: inputConfig.scanInputs.filter(
...
function ScannerConstructor(itemSelector, scanActiveClass, options) {
    ...
    function parseOptions(options) {
        if (options) {
            ...
            minBinarySplitThreshold =
                options.minBinarySplitThreshold || minBinarySplitThreshold;
            scanInactiveClass = options.scanInactiveClass || scanInactiveClass;

            scanVertical =
                options.scanVertical != undefined
                    ? options.scanVertical
                    : scanVertical;
            scanBinary =
                options.scanBinary != undefined
                    ? options.scanBinary
                    : scanBinary;
            touchScanning =
                options.touchScanning != undefined
                    ? options.touchScanning
                    : touchScanning;
            scanTimeoutFirstElementFactor =
        ...
        if (options.inputEventSelect) {
            ...
            _inputEventHandler.onInputEvent(
                new InputEventKey({ keyCode: 32 }),
                thiz.select
            ); //space as default key
        }
    ...
    function getNextGroup(remainingElements, allElements, scanVertical) {
        ...
        var group = compareElements.filter(
            (item) =>
                (firstMinPos >= minPosFn(item) &&
                    firstMinPos <= maxPosFn(item)) || //minimum of first value is inbetween the size of the item
                (minPosFn(item) <= firstMaxPos &&
                    maxPosFn(item) >= firstMaxPos) || //maximum of first value is inbetween the size of the item
        ...
        for (var i = 0; i < groups.length; i++) {
            ...
            if (group.length == 1) {
                var addToLastGroup =
                    ...
                    (lastGroup &&
                        nextGroup &&
                        lastGroup.length < nextGroup.length);
    ...
    function scan(elems, options) {
        ...
        if (_isScanning) {
            ...
            if (autoScan) {
                let timeout =
                    index === 0 && elems.length > 2
                        ? scanTimeoutMs * scanTimeoutFirstElementFactor
                        : scanTimeoutMs;
    ...
    thiz.select = function () {
        if (_isScanning) {
            ...
            if (_currentActiveScanElements.length > 1) {
                ...
                scan(
                    spitToSubarrays(L.flattenArray(_currentActiveScanElements))
                );
            } else {
                if (_selectionListener) {
                    _selectionListener(
                        L.flattenArray(_currentActiveScanElements)[0]
                    );
                }
                thiz.restartScanning();
            }
