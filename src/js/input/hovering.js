...
function HoverConstructor(itemSelector, options) {
    ...
    var _hoverTimeoutMs =
        options.timeoutMs !== undefined ? options.timeoutMs : 1000;
    ...
    thiz.startHovering = function () {
        ...
        let alreadyActivatedTTS = stateService.getState(
            constants.STATE_ACTIVATED_TTS
        );
        if (
            ...
            MainVue.setTooltip(
                i18nService.t("tapOnAnyElementToActivateSpeech")
            );
