...
function ClickerConstructor(itemSelector, options) {
    ...
    function onMouseDown(event) {
        ...
        if (
            !_enableMouseDown ||
            inputEventHandler.global.hasIncompleteTouchEvent()
        ) {
            return;
        }
        ...
        _elements.forEach(function (item) {
            if (options.useSingleClick && options.useMousedownEvent) {
                ...
                item.addEventListener(
                    "touchend",
                    inputEventHandler.global.resetIncompleteTouchEvent
                ); //needed because 'touchend' on document does not fire (but on element does) if element removed, see https://bugs.chromium.org/p/chromium/issues/detail?id=464579 and https://jsfiddle.net/jq3L6xo4/4/
