...
function Constructor() {
    ...
    thiz.destroy = function () {
        ...
        allInstances = allInstances.filter(
            (instance) => instance.getID() !== thiz.getID()
        );
    };
    ...
    function mouseMoveListener(event) {
        if (
            event.clientY < mouseBorderThreshold ||
            event.clientX < mouseBorderThreshold
        ) {
    ...
    function touchMoveListener(event) {
        ...
        if (!_touchMoveBeginPosY || !_touchMoveBeginPosX) {
            ...
            _touchMoveBeginPosY + touchMoveLength
        ) {
            ...
            _touchMoveBeginPosX + touchMoveLength
        ) {
    ...
    function keyboardListener(event) {
        ...
        if (keyHandlers[key]) {
            ...
            entries = entries.sort(
                (a, b) =>
                    a.counter - b.counter ||
                    a.inputEvent.repeat - b.inputEvent.repeat
            );
            entries.forEach((entry) => {
                entry.inputEvent.timeout =
                    entry.inputEvent.repeat > 1
                        ? entry.inputEvent.timeout || 500
                        : entry.inputEvent.timeout;
                if (
                    timeoutPassed(entry.lastKeydown, entry.inputEvent.timeout)
                ) {
                    entry.counter = 0;
                }
            });
            entries.forEach((entry) => {
                ...
                if (
                    ...
                    let existsConflictingHold =
                        entries.filter(
                            (e) => e !== entry && e.inputEvent.holdDuration
                        ).length > 0;
                    let conflictingRepeat = entries.filter(
                        (e) => e !== entry && e.inputEvent.repeat > 1
                    );
                    ...
                    if (existsConflictingHold) {
                        ...
                        existsConflictingRepeat &&
                        maxConflictingCounter < 1
                    ) {
                        let maxTimeout = Math.max(
                            ...conflictingRepeat.map(
                                (item) => item.inputEvent.timeout
                            )
                        );
