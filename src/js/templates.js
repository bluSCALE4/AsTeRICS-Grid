...
function getGridElementNormal(gridElem, fallbackLocale, metadata) {
    ...
    var template = `
<li class="item" data-w="${gridElem.width}" data-h="${
        gridElem.height
    }" data-x="${gridElem.posX}" data-y="${gridElem.posY}" data-id="${
        gridElem.id
    }" data-label="${label}" data-img-id="${imgId}" data-type="${
        gridElem.type
    }">
    <div class="grid-item-content" tabindex="40" aria-label="${ariaLabel}" id="${
        gridElem.id
    }" data-id="${gridElem.id}" data-empty="${
        !label && !imgData
    }" style="${`background-color: ${backgroundColor}; border: 1px solid ${getBorderColor(
        metadata
    )}`}">
        ...
        <div class="text-container" style="${
            txtContainerStyle + `color: ${fontColor}`
        }"><span>${label}</span></div>
...
function getGridElementPredict(gridElem, metadata) {
    gridElem = fillDefaultValues(gridElem);
    let txtContainerStyle =
        "display: table; height: 100%; text-align: center; width: 100%";
    let label = i18nService.getTranslation(gridElem.label);
...
<li class="item" data-w="${gridElem.width}" data-h="${
        ...
        gridElem.id
    }" data-label="${label}" data-type="${gridElem.type}">
    <div class="grid-item-content" tabindex="40" id="${gridElem.id}" data-id="${
        gridElem.id
    }" style="${`background-color: rgb(255,228,178); border: 1px solid ${getBorderColor(
        metadata
    )}`}">
...
function getGridElementYTPlayer(gridElem, metadata) {
    gridElem = fillDefaultValues(gridElem);
    let stopClicking =
        gridElem.additionalProps[GridElement.PROP_YT_PREVENT_CLICK];
    let label = i18nService.getTranslation(gridElem.label);
...
<li class="item" data-w="${gridElem.width}" data-h="${
    ...
    <div class="grid-item-content" tabindex="40" aria-label="${i18nService.t(
        "ELEMENT_TYPE_YT_PLAYER"
    )}" id="${gridElem.id}" data-id="${
        gridElem.id
    }" style="${`border: 1px solid ${getBorderColor(metadata)}`}">
        ${
            stopClicking
...
function getHintsElement(gridElem) {
    let hiddenHint = gridElem.hidden
        ? '<i class="fas fa-eye-slash element-hint"></i>'
        : "";
    let navHint =
        gridElem.actions.filter(
            (a) => a.modelName === GridActionNavigate.getModelName()
        ).length > 0
            ? '<i class="fas fa-sticky-note fa-rotate-180 fa-flip-vertical element-hint"></i>'
            : "";
    return `<span style="position: absolute; right: 0; color: #5a717a">${
        hiddenHint + " " + navHint
    }</span>`;
}
