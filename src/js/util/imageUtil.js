...
imageUtil.getBase64FromImg = function (img, maxWidth, quality, mimeType) {
    ...
    mimeType =
        mimeType || img.src.indexOf("data:") === 0
            ? img.src.substring(5, img.src.indexOf(";"))
            : null;
    mimeType = mimeType || (img.src.indexOf(".png") > -1 ? "image/png" : null);
    mimeType =
        mimeType || (img.src.indexOf(".svg") > -1 ? "image/svg+xml" : null);
...
imageUtil.convertBase64 = function (originalBase64, maxWidth, quality) {
    return new Promise((resolve, reject) => {
        ...
        if (
            originalBase64.substring(5, originalBase64.indexOf(";")) ===
            "image/svg+xml"
        ) {
        ...
        img.onload = function () {
            try {
                resolve(
                    imageUtil.getBase64FromImg(img, maxWidth, quality).data
                );
            } catch (e) {
                resolve(null);
            }
...
imageUtil.urlToBase64 = function (url, maxWidth, mimeType) {
    return imageUtil
        .urlToBase64WithDimensions(url, maxWidth, mimeType)
        .then((dataWithDim) => {
        ...
        (html2canvas) => {
            return html2canvas
                .default(document.querySelector(selector), {
                    ...
                    ignoreElements: (node) => {
                        return (
                            constants.IS_FIREFOX &&
                            node.style["background-image"].indexOf(
                                "image/svg"
                            ) !== -1
                        );
                    }
