...
function MapCache() {
    ...
    thiz.set = function (key, value, objectType) {
        ...
        if (objectType && typeof objectType !== "function") {
            log.warn(
                "object type has to be a constructor function or empty, aborting."
            );
            return;
        }
        if (firstValue && firstValue.isShortVersion) {
            log.debug(
                "not caching model instances only containing short version of data, aborting."
            );
            return;
        }
    ...
    thiz.get = function (key) {
        ...
        return isArray
            ? value.map((e) => instantiate(objectType, e))
            : instantiate(objectType, value);
    };
