...
fontUtil.getFontSizePx = function (elem, oneLine) {
    ...
    var fontSize1 = Math.floor(
        Math.sqrt((areaElem * 0.5) / Math.max(15, label.length))
    );
...
fontUtil.adaptFontSize = function (elems) {
    for (var i = 0; i < elems.length; i++) {
        ...
        if (textContainerElem) {
            textContainerElem.style.fontSize = fontUtil.getFontSizePx(
                $(elem),
                oneLine
            );
        }
...
fontUtil.getTextWidth = function (text, containerElem, targetSize) {
    ...
    const canvas =
        fontUtil.getTextWidth.canvas ||
        (fontUtil.getTextWidth.canvas = document.createElement("canvas"));
...
fontUtil.getHighContrastColor = function (
    hexBackground,
    lightColor,
    darkColor
) {
